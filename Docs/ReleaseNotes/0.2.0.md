...
- **Breaking Change**: Module renamed from `AzurePipelinesUtils` to `PipelinesUtils` to reflect broader platform support
...
If you're currently using `AzurePipelinesUtils`, follow these steps to migrate:

1. **Uninstall the old module:**
   ```powershell
   Uninstall-Module AzurePipelinesUtils
   ```

2. **Install the new module:**
   ```powershell
   Install-Module PipelinesUtils
   ```

3. **Update import statements in your scripts:**
   ```powershell
   # Old
   Import-Module AzurePipelinesUtils
